...
// @version      1.9.9
// @description  Draggable Covidence panel with saved position, decision logging, CSV export, color-coded decision display, in-panel decision buttons, foldable summary section, and clickable jump-to links.
// @match        *://*.covidence.org/*
// @grant        GM_setValue
// @grant        GM_getValue
// ==/UserScript==
...
(function () {
    ...
    function createPanel() {
        ...
        panel.innerHTML = `
            ...
            <textarea id='studyListInput' rows='6' style='width:100%; margin-top:10px; font-size: 15px;' placeholder='Enter studies to screen, e.g., 1,2,3.'></textarea>
    ...
    GM_setValue('summaryVisible', isHidden);
        ...
        function updateSummary() {
            function makeClickableList(ids, color) {
                return ids.map(id => {
                    return `<a href="#" style="color:${color}; text-decoration:underline;" onclick="document.querySelector('#covidence-panel').dataset.jumpTo='${id}'; return false;">${id}</a>`;
                }).join(', ');
            }
            ...
            summaryList.innerHTML =
                `<strong style="color:orange;">Maybe:</strong> ${makeClickableList(maybeList, 'orange')}<br>
                 ...
                 <strong style="color:red;">No:</strong> ${makeClickableList(noList, 'red')}`;
        }

        setInterval(() => {
            const panel = document.querySelector('#covidence-panel');
            if (panel && panel.dataset.jumpTo) {
...
function simulateDecision(value) {
    ...
    if (index < studies.length - 1) {
        ...
        setTimeout(() => {
            if (allDecisionsMade()) alert("All studies completed!");
        }, 1000);
    }
}

function allDecisionsMade() {
    return studies.every(id => decisions.hasOwnProperty(id));
}
...
nextBtn.onclick = function () {
    if (index < studies.length - 1) {
        ...
        updateStudy();
        simulateEnter();
    } else if (index === studies.length - 1 && allDecisionsMade()) {
