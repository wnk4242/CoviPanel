...
(function() {
    ...
    function updateLifetimeProgressUI() {
        ...
        if (display) {
            display.innerHTML = "";
            const achievedLevels = LEVELS.filter(lvl => count >= lvl.threshold);
            const highest = achievedLevels[achievedLevels.length - 1];
            if (highest) {
                const div = document.createElement("div");
                div.textContent = `${highest.emoji} ${highest.title}`;
