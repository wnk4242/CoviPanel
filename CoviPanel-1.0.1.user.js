...
(function() {
	...
	const LEVELS = [{
			threshold: 0,
			title: "Research Assistant I 🤦‍♂️"
		},
		{
			threshold: 50,
			title: "Research Assistant II 🤷‍♂️"
		},
		{
			threshold: 100,
			title: "PhD Student 1 Yr. 👦"
		},
		{
			threshold: 200,
			title: "PhD Student 4 Yr. 🧔"
		},
		{
			threshold: 300,
			title: "PhD Candidate 👴"
	...
	const LEVEL_DETAILS = {
		"Research Assistant I 🤦‍♂️": {
			desc: "Doesn't understand research at all.",
			emoji: "🤦‍♂️"
		},
		"Research Assistant II 🤷‍♂️": {
			desc: "Still confused but trying.",
			emoji: "🤷‍♂️"
		},
		"PhD Student 1 Yr. 👦": {
			desc: "Newbie in grad school.",
			emoji: "👦"
		},
		"PhD Student 4 Yr. 🧔": {
			desc: "Has a beard of experience.",
			emoji: "🧔"
		},
		"PhD Candidate 👴": {
			desc: "Old and wise (but still poor).",
			emoji: "👴"
		},
		"Dr. 👨‍🎓": {
			desc: "Finally got the title, still no job.",
			emoji: "👨‍🎓"
		},
		"Postdoc 1 Yr. 🙂": {
			desc: "Optimistic researcher.",
			emoji: "🙂"
		},
		"Postdoc 9 Yr. 😭": {
			desc: "Send help.",
		...
		"Assistant Prof. 👨‍🏫": {
			desc: "Grading forever.",
			emoji: "👨‍🏫"
		},
		"Associate Prof. 🦹‍♂️": {
			desc: "Master of committees.",
			emoji: "🦹‍♂️"
		},
		"Professor 🧙‍♂️": {
			desc: "Wizard of academia.",
			emoji: "🧙‍♂️"
		},
		"Emeritus Prof. 🥂": {
			desc: "Retired. Toasting to freedom.",
			emoji: "🥂"
		}
	};
  ...
  `;
		...
		const options = [{
			...
			},
			{
				...
				fn: () => {
					...
					if (decisionSummary) {
						const show = decisionSummary.style.display === "none";
						decisionSummary.style.display = show ? "" : "none";
						GM_setValue("showDecisionCounts", show);
					}
				},
				key: "showDecisionCounts"
			},
			{
				label: "A.I. assistant",
				fn: () => {
		...
		options.forEach(({
			...
			const isChecked = GM_getValue(key, true);
    ...
    `;
			row.addEventListener("mouseenter", () => {
				row.style.backgroundColor = "#eee"; // light grey
			});
			...
			// Toggle when either checkbox or label is clicked
			const toggle = () => {
	...
	function createLevelProgressUI(parent) {
		...
		container.style.margin = "14px 0 15px 0";
		const count = parseInt(GM_getValue("totalStudiesScreened", "0"), 10);
...
<div id="lifetimeRankTitleWrapper" style="flex: 1; text-align: center; position: relative;">
  ...
  <div id="rankTooltip" style="display:none; position:absolute; top:120%; left:50%; transform:translateX(-50%); background:#fff; border:1px solid #ccc; border-radius:6px; padding:6px 10px; box-shadow:0 4px 8px rgba(0,0,0,0.1); white-space:nowrap; z-index:9999; font-size:13px; text-align:center;"></div>
            ...
            <button id="resetProgressBtn" title="Reset your title" style="background: none; border: none; cursor: pointer; font-size: 14px; color: #888;">🔄</button>
        </div>
...
<div id="covidence-header" style="font-size: 15px; font-weight: bold; cursor: move;">
  CoviPanel 1.0 <span id="contactInfoIcon" title="Contact me" style="cursor: pointer;">💬</span>
</div>
  <div id="topRightIcons" style="display: none; gap: 7px; flex-direction: row; align-items: center;">
    ...
    <button id="uploadSkinBtn" title="Edit button appearance" style="background:none; border:none; cursor:pointer; font-size:17px;" aria-label="Panel skin options">📷</button>
<input type="file" id="skinImageInput" accept=".jpg,.jpeg,.png,.gif" style="display:none;">
    <button id="exportBtn" title="Click to save | Hold to restart" style="background:none; border:black; cursor:pointer; font-size:21px;" aria-label="Save and restart ">🖫</button>
       <button id="resetBtn" style="display: none;"></button>
            ...
            <div id='studyControls' style='display:none; margin-top:15px;'>
              <div style="margin-bottom: 5px;">Study ID: <span id='currentStudy'>?</span><span id='progressInline' style='margin-left: 8px; font-size: 14px; color: #555;'></span>
...
<div style='margin-top: -15px; margin-bottom: 0px;'>
  <div style='display: flex; gap: 6px; align-items: center;'>
    ...
    <button id='addKeywordsBtn' title='Search keywords' style='background: none; border: none; cursor: pointer; font-size: 18px;'>🔍︎</button>
    <span id='summaryToggleIcon' title='Advanced features'>⋯</span>
  </div>
...
`;
		...
		// 💬 Contact info popup
		document.getElementById("contactInfoIcon").addEventListener("click", function(e) {
			// Remove any existing popup
			document.getElementById("contactInfoPopup")?.remove();

			const popup = document.createElement("div");
			popup.id = "contactInfoPopup";
			popup.innerHTML = `
    <strong>Contact Info</strong><br>
    📧 Email: wangnaike@tamu.edu<br>
    💻 GitHub: <a href="https://github.com/wnk4242" target="_blank">wnk4242</a><br>
    💼 LinkedIn: <a href="https://www.linkedin.com/in/wangnaike/" target="_blank">wangnaike</a>
  `;
			const iconRect = e.target.getBoundingClientRect();
			popup.style.position = "absolute";
			popup.style.left = `${iconRect.left + window.scrollX -180}px`;
			popup.style.top = `${iconRect.bottom + window.scrollY - 120}px`;
...
`);
		...
		summaryList.addEventListener('click', function(e) {
			if (e.target.id === 'resetProgressBtn') {
				if (confirm('Reset your title and level? This will not affect your progress in Covidence.')) {
		...
		function updateSummary() {
			...
			Object.entries(decisionsMap).forEach(([label, {
				color,
				list
			}]) => {
...
<button id="runAIButton" style="
  ...
  text-align: center;
  flex-grow: 1;
">🤖 Ask AI about this study</button>

<div id="togglePromptInputs" title="AI prompt settings" style="
  font-size: 16px;
  color: #007BFF;
...
">⚙️</div>
    ...
    <div id="aiInputsWrapper" style="display: none;">
     ...
     <label style="display:block; margin-top:6px;">Study eligibility criteria (required):</label>
      <textarea id="inclusionCriteriaInput" rows="3" style="width:100%; font-size:13px; margin-bottom:6px;" placeholder="Provide inclusion and exclusion criteria. AI will use them to suggest screening decisions."></textarea>

     <label style="display:block; margin-top:6px;">General prompt for AI (optional):</label>
      <textarea id="systemPromptInput" rows="3" style="width:100%; font-size:13px; margin-bottom:6px;" placeholder="e.g., You are a senior researcher in X area. Only answer with yes, no, or maybe."></textarea>
...
<div style="display: flex; gap: 6px; align-items: center;">
  <button id="loadAPIKeyBtn" style="
    ...
    cursor: pointer;
    line-height: 1;
  ">📂 Load API key from local file</button>
...
`);
			...
			if (index < studies.length - 1) {
				...
				const allFinished = studies.every(id => ["Yes", "No", "Maybe"].includes(decisions[id]));
...
`);
					...
					setTimeout(() => {
						...
						alert(
							"You've screened all the studies for this session!" + sessionMsg +
    ...
    cursor: pointer;
  `;
			menu.textContent = "⟳ Save and restart";
			menu.onmouseenter = () => menu.style.background = "#eee";
			menu.onmouseleave = () => menu.style.background = "white";
            menu.onclick = () => {
                document.getElementById("floatingExportMenu")?.remove();

                if (!confirm("Are you sure you want to export your decisions and start a new session?")) {
                    return;
                }
                ...
                setTimeout(() => {
                    ...
                    if (match) {
                        ...
                        location.reload(); // fallback just in case
                    }
                }, 300);
            };
...
`;
	...
	document.body.addEventListener("click", function(e) {
		...
		if (voteBtn && ["Yes", "No", "Maybe"].includes(voteBtn.value)) {
			...
			const allFinished = studies.every(id => ["Yes", "No", "Maybe", "Skipped"].includes(decisions[id]));
