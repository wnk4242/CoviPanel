...
function checkAndToggleLEDEffect() {
  ...
  // Only trigger LED effect once, the first time both criteria are satisfied
  if (runBtn) {
    if (inclusionText && hasAPIKey && !ledEffectTriggered) {
      runBtn.classList.add("led-effect");
      ledEffectTriggered = true;
      GM_setValue("ledEffectTriggered", true);
    } else if (!inclusionText || !hasAPIKey) {
      runBtn.classList.remove("led-effect");
      ledEffectTriggered = false;
...
@keyframes ledFlashLimited {
  0%, 100% { box-shadow: 0 0 0px 0px rgba(0,255,0,0.8); }
  50% { box-shadow: 0 0 12px 6px rgba(0,255,0,0.5); }
}

#runAIButton.led-effect {
