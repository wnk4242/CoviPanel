...
(function() {
    ...
    style.textContent = '#covidence-panel { visibility: hidden; }';
    document.head.appendChild(style);
        ...
        panel.innerHTML = `
            ...
            <div id='studyControls' style='display:none; margin-top:15px; position:relative;'>
  ...
  <button id='skipBtn' title='Skip this study' style='margin-left: 6px; background: none; border: none; font-size: 16px; cursor: not-allowed; opacity: 0.5;' disabled>⏭</button>
             ...
             <div id="badgeContainer" style="position:absolute; bottom:-30px; left:-6px; display:flex; gap:4px; font-size:20px; pointer-events: auto;"></div>


            </div>`;

        document.body.appendChild(panel);
document.getElementById("covidence-panel").style.visibility = "visible";
...
if (progressInline) progressInline.textContent = `(${counted} of ${studies.length} done)`;
        ...
        function attachDecisionListeners() {
            ...
            buttons.forEach(btn => {
                ...
                if (studies.every(id => ["Yes", "No", "Maybe", "Skipped"].includes(decisions[id]))) {
                    updateSummary();
                    if (progressInline) progressInline.textContent = `(${counted} of ${studies.length} done)`;
                    ...
                    setTimeout(() => {
                        ...
                        if (panel) {
                            ...
                            const startBtn = panel.querySelector('#startBtn');
                            const controls = panel.querySelector('#studyControls');
                            const detectBtn = document.getElementById("detectIDsBtn");
