    ...
    GM_setValue('summaryVisible', isHidden);
        ...
        function updateStudy() {
            if (index < studies.length) {
                ...
                const plainID = studies[index];
  runAIAssistantForStudy(plainID); // 👈 Automatically fetch AI decision
                const searchBox = document.querySelector("input[placeholder='Search studies']");
                if (searchBox) {
                    searchBox.focus();
...
<div id="aiRequestCounters" style="margin-top: 10px; font-size: 12px; color: #444;">
  <div>🤖 Auto requests: <span id="autoCount">0</span></div>
  <div>🧠 Manual requests: <span id="manualCount">0</span></div>
  <button id="resetAICountsBtn" style="margin-top:6px; font-size:12px;">♻️ Reset AI Counters</button>
</div>
...
if (openaiKeyFromFile) {
  content.querySelector("#apiKeyStatus").textContent = "✅ API key loaded from file (saved)";
    ...
    // ✅ Handle reset button
const resetBtn = content.querySelector("#resetAICountsBtn");
if (resetBtn) {
  resetBtn.onclick = () => {
    GM_setValue("chatgpt_auto_requests", "0");
    GM_setValue("chatgpt_manual_requests", "0");
    if (autoEl) autoEl.textContent = "0";
    if (manualEl) manualEl.textContent = "0";
    alert("✅ AI request counters have been reset.");
  };
}

// ✅ Update the request counters in the UI
const auto = GM_getValue("chatgpt_auto_requests", "0");
const manual = GM_getValue("chatgpt_manual_requests", "0");
const autoEl = content.querySelector("#autoCount");
const manualEl = content.querySelector("#manualCount");
...
content.querySelector("#runAIButton").onclick = async () => {
    // ✅ Increment manual AI request counter
const prevManual = parseInt(GM_getValue("chatgpt_manual_requests", "0"), 10);
GM_setValue("chatgpt_manual_requests", (prevManual + 1).toString());
console.log("✅ Manual counter incremented");
...
async function runAIAssistantForStudy(studyID) {
    // ✅ Increment automatic AI request counter
const prevAuto = parseInt(GM_getValue("chatgpt_auto_requests", "0"), 10);
GM_setValue("chatgpt_auto_requests", (prevAuto + 1).toString());
console.log("✅ Auto counter incremented");
  const criteria = GM_getValue("inclusionCriteriaText", "").trim();
  const systemPrompt = GM_getValue("customSystemPrompt", "You are an assistant screening research abstracts for inclusion based on user-defined criteria. Keep explanations short and simple.");
  if (!openaiKeyFromFile || !criteria) return;
  ...
  try {
    ...
    const aiDecisions = JSON.parse(GM_getValue("chatgpt_decisions", "{}"));
    const aiExplanations = JSON.parse(GM_getValue("chatgpt_explanations", "{}"));
    aiDecisions[studyID] = decisionOnly;
    aiExplanations[studyID] = reply;
