...
`);
    ...
    if (confirm('Reset your lifetime progress?')) {
        ...
        GM_setValue("totalTimeMs", "0");// ⏱ Reset total time
...
const maybeCount = decisionsMap.Maybe.list.length;
const yesCount = decisionsMap.Yes.list.length;
const noCount = decisionsMap.No.list.length;

const chartWrapper = document.createElement("div");
chartWrapper.style.display = "flex";
chartWrapper.style.alignItems = "center";
chartWrapper.style.justifyContent = "center";
chartWrapper.style.gap = "14px";
chartWrapper.style.marginTop = "10px";

const chartCanvas = document.createElement("canvas");
chartCanvas.id = "decisionPieChart";
chartCanvas.width = 130;
chartCanvas.height = 130;

const legendBox = document.createElement("div");
legendBox.innerHTML = `
  <div style="display: flex; align-items: center; margin-bottom: 6px;">
    <div style="width: 10px; height: 10px; background-color: #ff9800; margin-right: 6px;"></div> Maybe
  </div>
  <div style="display: flex; align-items: center; margin-bottom: 6px;">
    <div style="width: 10px; height: 10px; background-color: #4caf50; margin-right: 6px;"></div> Yes
  </div>
  <div style="display: flex; align-items: center;">
    <div style="width: 10px; height: 10px; background-color: #f44336; margin-right: 6px;"></div> No
  </div>
`;
legendBox.style.fontSize = "12px";

chartWrapper.appendChild(chartCanvas);
chartWrapper.appendChild(legendBox);
summaryList.appendChild(chartWrapper);




    const ctx = document.getElementById("decisionPieChart").getContext("2d");
if (window.decisionPieChart) {
    window.decisionPieChart.destroy();
}
window.decisionPieChart = new Chart(ctx, {
    type: 'pie',
    data: {
        labels: ['Maybe', 'Yes', 'No'],
        datasets: [{
            data: [maybeCount, yesCount, noCount],
            ...
            tooltip: {
                callbacks: {
                    label: function (context) {
                        const total = maybeCount + yesCount + noCount;
...
alert("You've reached the end of your study list!" + sessionMsg);
...
if (startTime) {
    ...
    const totalS = totalSec % 60;
    const totalTimeStr = `${totalH}h ${totalM}m ${totalS}s`;
const totalSessions = parseInt(GM_getValue("sessionCount", "0"), 10);
