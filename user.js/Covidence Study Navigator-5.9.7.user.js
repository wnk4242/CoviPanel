...
aiSection.style.marginTop = "10px";
aiSection.innerHTML = `
  <details style="font-size: 13px; border-top: 1px solid #ccc; padding-top: 8px;">
    <summary><strong>🤖 AI Assistant (Advanced)</strong></summary>
...
`;
        ...
        function updateStudy() {
            ...
            summaryList.appendChild(aiSection);
            summaryList.appendChild(aiSection);
...
`;
// Restore open state of AI panel and track changes
setTimeout(() => {
  const details = aiSection.querySelector("details");
  if (details) {
    const shouldBeOpen = GM_getValue("aiDetailsOpen", false);
    details.open = shouldBeOpen;

    details.addEventListener("toggle", () => {
      GM_setValue("aiDetailsOpen", details.open);
    });
  }
}, 0);

        // Save inclusion criteria on input
setTimeout(() => {
  const criteriaInput = document.getElementById("inclusionCriteriaInput");
  if (criteriaInput) {
...
document.getElementById("loadAPIKeyBtn").onclick = () => {
    document.getElementById("apiKeyFileInput").click();
};

        document.getElementById("runAIButton").onclick = async () => {
    const key = openaiKeyFromFile;
    const criteria = document.getElementById("inclusionCriteriaInput").value.trim();
    const output = document.getElementById("aiDecisionOutput");
    const currentID = studies[index];

    if (!key || !criteria) return alert("Please load your API key and enter inclusion criteria.");
    output.innerHTML = "⏳ Asking ChatGPT...";

    const abstract = extractAbstractText();
    if (!abstract) return output.innerHTML = "❌ Could not extract abstract.";
  ...
  {
    role: "system",
    content: "You are an assistant screening research abstracts for inclusion based on user-defined criteria. Keep explanations short and simple."
  },
  {
    role: "user",
    content: `Abstract: ${abstract}\n\nInclusion Criteria: ${criteria}\n\nShould this study be included in the review? Reply with your decision (Yes, No, or Maybe), followed by a short explanation using simple words in no more than 3 sentences.`
  }
],
        ...
        console.error(e);
        output.innerHTML = "❌ Error calling ChatGPT.";
    }
};


document.getElementById("apiKeyFileInput").addEventListener("change", (event) => {
...
reader.onload = (e) => {
    ...
    document.getElementById("apiKeyStatus").textContent = "✅ API key loaded from file.";
};

    reader.onerror = () => {
        document.getElementById("apiKeyStatus").textContent = "❌ Failed to read API key file.";
    };
    reader.readAsText(file);
});
        // Restore saved API key
const savedKey = GM_getValue("openaiKeyFromFile", "");
if (savedKey) {
    openaiKeyFromFile = savedKey;
    document.getElementById("apiKeyStatus").textContent = "✅ API key loaded from file (saved)";
}

// Restore saved inclusion criteria
const savedCriteria = GM_getValue("inclusionCriteriaText", "");
if (savedCriteria) {
    const criteriaInput = document.getElementById("inclusionCriteriaInput");
    if (criteriaInput) {
        criteriaInput.value = savedCriteria;
    }
}
...
`;
    ...
    `<strong style="color:orange;">Maybe:</strong> ${makeColoredList(maybeList, 'orange')}<br>
     ...
     <strong style="color:red;">No:</strong> ${makeColoredList(noList, 'red')}</div>`;
                    createLevelProgressUI(summaryList);
summaryList.appendChild(aiSection);
// 🔁 Restore AI panel open state
const aiDetails = aiSection.querySelector("details");
if (aiDetails) {
    const shouldBeOpen = GM_getValue("aiDetailsOpen", false);
    aiDetails.open = shouldBeOpen;
}
